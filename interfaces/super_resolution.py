...
# ============================================
# CONFIGURATION: Enable advanced visuals in base
# ============================================
ENABLE_ADVANCED_FEATURES = True  # Always on for academic-level visuals
ENABLE_ENHANCED_VIS = True  # Enhanced baseline visualizations
# ============================================

if ENABLE_ADVANCED_FEATURES:
    from utils.visualization import plot_training_curves, plot_comparison_table, plot_diffusion_features, plot_prediction_examples, plot_metrics_scatter

if ENABLE_ENHANCED_VIS:
    try:
        from utils.visualization_enhanced import (
            plot_training_dynamics, plot_feature_maps, plot_attention_maps,
            plot_super_resolution_comparison, plot_gradient_flow, create_visualization_dashboard
        )
    except ImportError as e:
        print(f"Warning: Enhanced visualizations not available: {e}")
        ENABLE_ENHANCED_VIS = False
...
class TextSR(base.TextBase):
    ...
    def train(self):
        ...
        # Add adaptive learning rate scheduler (only if advanced features enabled)
        if ENABLE_ADVANCED_FEATURES:
        ...
        train_metrics_path = os.path.join(cfg.ckpt_dir, "train_metrics.csv")
        if not os.path.exists(train_metrics_path):
            with open(train_metrics_path, 'w', newline='') as train_file:
                writer = csv.writer(train_file)
                writer.writerow([
                    'timestamp', 'epoch', 'batch_index', 'batches_per_epoch',
                    'iteration', 'loss', 'learning_rate', 'gradient_norm', 'model'
                ])
        ...
        for epoch in range(cfg.epochs):
            for j, data in enumerate(train_loader):
                ...
                # Only compute auxiliary recognition loss if logits are available
                ...
                model_label = getattr(self, '_comparison_name', 'PEAN')
                with open(train_metrics_path, 'a', newline='') as train_file:
                    writer = csv.writer(train_file)
                    writer.writerow([
                ...
                if iters % cfg.VAL.valInterval == 0:
                    ...
                    # Collect data for enhanced visualizations
                    sr_comparison_data = {'lr': [], 'sr': [], 'hr': [], 'labels': [], 'preds': [], 'psnr': [], 'ssim': []}
                    
                    for k, val_loader in enumerate(val_loader_list):
                        data_name = self.config.TRAIN.VAL.val_data_dir[k].replace('\\', '/').split('/')[-1]
                        model_label = getattr(self, '_comparison_name', 'PEAN')
                        print('='*100)
                        print(f'evaling {data_name} [{model_label}]')
                        # Collect examples only if advanced features enabled
                        if ENABLE_ADVANCED_FEATURES:
                            collect_examples = (k == 0)
                            metrics_dict = self.eval(model, val_loader, epoch, parseq, collect_examples=collect_examples)
                        else:
                            metrics_dict = self.eval(model, val_loader, epoch, parseq, collect_examples=False)
                        ...
                        # Record validation metrics and collect examples
                        ...
                        self.metrics_history[f'cer_{data_name}'].append(metrics_dict.get('cer_avg', 0.0))
                        
                        # Collect examples and diffusion states from first dataset
                        if ENABLE_ADVANCED_FEATURES and collect_examples:
                    ...
                    # Advanced features: scheduler and visualizations (only if enabled)
                    if ENABLE_ADVANCED_FEATURES:
                        ...
                        try:
                            ...
                            plot_metrics_scatter(self.metrics_history, save_path=os.path.join(vis_dir, 'metrics_scatter.png'))
                            ...
                            plot_metrics_scatter(self.metrics_history, save_path=os.path.join(latest_dir, 'metrics_scatter.png'))
                            if len(all_diffusion_states) > 0:
                                timesteps = [1000, 500, 0][:len(all_diffusion_states)]
                                plot_diffusion_features(all_diffusion_states[:3], timesteps, 
                    ...
                    # Enhanced visualizations for baseline
                    if ENABLE_ENHANCED_VIS and iters > 0:
                        try:
                            self.vis_counter += 1
                            vis_dir_enhanced = os.path.join(cfg.ckpt_dir, 'visualizations_enhanced')
                            os.makedirs(vis_dir_enhanced, exist_ok=True)
                            
                            # Create comprehensive dashboard
                            dashboard_dir = create_visualization_dashboard(
                                self.metrics_history,
                                save_dir=vis_dir_enhanced
                            )
                            
                            # Gradient flow visualization (every 3 validations to avoid overhead)
                            if self.vis_counter % 3 == 0:
                                plot_gradient_flow(
                                    model.named_parameters(),
                                    save_path=os.path.join(dashboard_dir, '04_gradient_flow.png')
                                )
                            
                            # SR comparison from collected data
                            for k, val_loader in enumerate(val_loader_list):
                                data_name = self.config.TRAIN.VAL.val_data_dir[k].replace('\\', '/').split('/')[-1]
                                # Use the sr_comparison data from the last validation
                                if 'sr_comparison' in metrics_dict and metrics_dict['sr_comparison']:
                                    samples = metrics_dict['sr_comparison'][:8]
                                    plot_super_resolution_comparison(
                                        [s['lr'] for s in samples],
                                        [s['sr'] for s in samples],
                                        [s['hr'] for s in samples],
                                        [s['label'] for s in samples],
                                        [s['pred'] for s in samples],
                                        [s['psnr'] for s in samples],
                                        [s['ssim'] for s in samples],
                                        save_path=os.path.join(dashboard_dir, f'05_sr_comparison_{data_name}.png')
                                    )
                                    break  # Only do it for first dataset to save time
                            
                            print(f'✓ Enhanced visualizations saved to: {dashboard_dir}')
                        except Exception as e:
                            print(f'Warning: Failed to generate enhanced visualizations: {e}')
                            import traceback
                            traceback.print_exc()
        ...
        # Determine which recognizer had the best overall performance
        best_overall_score = -1
        best_recognizer = None
        
        for rec in ['aster', 'crnn', 'moran']:
            score = (best_history_acc[f'easy_{rec}'] * 1619 + 
                    best_history_acc[f'medium_{rec}'] * 1411 + 
                    best_history_acc[f'hard_{rec}'] * 1343) / (1619 + 1411 + 1343)
            print(f'Best {rec.upper()} overall: {score:.4f}')
            if score > best_overall_score:
                best_overall_score = score
                best_recognizer = rec
        
        print(f'Best overall recognizer: {best_recognizer.upper()} with score {best_overall_score:.4f}')
        self.save_final_model(model, f'sum_{best_recognizer}')
        self.diffusion.save_final_model(f'sum_{best_recognizer}')
        print('='*110 + '\n')
    ...
    def eval(self, model, val_loader, index, parseq, collect_examples=False):
        ...
        sr_comparison_samples = []  # For enhanced SR comparison visualization
        ...
        for _, data in (enumerate(val_loader)):
            ...
            for pred, target in zip(pred_str_sr_aster, label_strs):
                if pred == str_filt(target, 'lower'):
                    n_correct_aster += 1
                # CER accumulation
                gt = str_filt(target, 'lower')
                total_char_edits += _levenshtein(pred, gt)
                total_char_count += max(1, len(gt))
            if crnn is not None:
            ...
            # Collect SR comparison data for enhanced visualization
            if ENABLE_ENHANCED_VIS and len(sr_comparison_samples) < 12:
                batch_psnr = self.cal_psnr(images_sr, images_hr)
                batch_ssim = self.cal_ssim(images_sr, images_hr)
                for i in range(min(val_batch_size, 12 - len(sr_comparison_samples))):
                    sr_comparison_samples.append({
                        'lr': images_lr[i].cpu(),
                        'sr': images_sr[i].cpu(),
                        'hr': images_hr[i].cpu(),
                        'label': str_filt(label_strs[i], 'lower'),
        ...
        # Show where images are saved for easier tracing
        try:
            save_dir = os.path.join(self.vis_dir, str(index))
            print(f'save display images to: {save_dir}')
        except Exception:
        ...
        print(f'aster_accuray: {accuracy_aster * 100:.2f}% | crnn_accuray: {accuracy_crnn * 100:.2f}% | moran_accuray: {accuracy_moran * 100:.2f}% ')
        ...
        metric_dict['cer_avg'] = round((total_char_edits / total_char_count) if total_char_count > 0 else 0.0, 6)
        ...
        # Add SR comparison data for enhanced visualization
        if ENABLE_ENHANCED_VIS and sr_comparison_samples:
            metric_dict['sr_comparison'] = sr_comparison_samples
