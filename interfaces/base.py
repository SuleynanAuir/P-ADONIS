...
class TextBase(object):
    ...
    def get_test_data(self, dir_):
        ...
        # Use deterministic order for testing/validation so cross-model pairing is consistent
        test_loader = torch.utils.data.DataLoader(
            test_dataset, batch_size=self.batch_size,
            shuffle=False, num_workers=int(cfg.workers), pin_memory=True,
    ...
    def generator_init(self, resume_this=''):
        ...
        def _load_weights_safe(m, path):
            print('loading PEAN (w/o TPEM) from %s ' % path)
            ...
            # try strict first, fallback to non-strict
            try:
                m.load_state_dict(sd, strict=True)
            except Exception as e:
                print('Warning: strict load failed, fallback to non-strict. Reason:', e)
        ...
        if resume_this != '':
            _load_weights_safe(model, resume_this)
        elif self.resume != '':
            _load_weights_safe(model, self.resume)
    ...
    def tripple_display(self, image_in, image_out, image_target, pred_str_lr, pred_str_sr, label_strs, index):
        # Get model name for filename prefix
        model_name = getattr(self, '_comparison_name', 'PEAN')
        ...
        for i in (range(min(image_in.shape[0], self.config.TRAIN.VAL.n_vis) )):
            ...
            if not os.path.exists(out_path):
                os.mkdir(out_path)
            # Add model name prefix to filename
            im_name = f'{model_name}_{pred_str_lr[i]}_{pred_str_sr[i]}_{label_strs[i]}_.png'
            # Remove characters that are invalid in Windows filenames
            invalid_chars = ['/', '\\', ':', '*', '?', '"', '<', '>', '|']
            for char in invalid_chars:
                im_name = im_name.replace(char, '_')
    ...
    def test_display(self, image_in, image_out, image_target, pred_str_lr, pred_str_sr, label_strs, str_filt, out_root):
        ...
        if not os.path.exists(out_root):
            os.mkdir(out_root)
        # Add model name prefix to filename
        im_name = f'{model_name}_{pred_str_lr}_{pred_str_sr}_{label_strs}_.png'
        # Remove characters that are invalid in Windows filenames
        invalid_chars = ['/', '\\', ':', '*', '?', '"', '<', '>', '|']
        for char in invalid_chars:
            im_name = im_name.replace(char, '_')
    ...
    def save_checkpoint(self, netG, epoch, iters, best_acc_dict, best_model_info, is_best, converge_list, metric="sum"):
        ckpt_path = self.config.TRAIN.ckpt_dir
        if not os.path.exists(ckpt_path):
            os.makedirs(ckpt_path, exist_ok=True)
        
        # Get model name from _comparison_name attribute or default to 'PEAN'
        model_name = getattr(self, '_comparison_name', 'PEAN').upper()
        
        if is_best:
            # Save best model with format: MODEL_metric_epochX.pth
            save_path = os.path.join(ckpt_path, '{}_{}_{}.pth'.format(model_name, metric, epoch))
            torch.save(netG.state_dict(), save_path)
            print(f'Best model saved: {save_path}')
        else:
            # Save periodic checkpoint with format: MODEL_checkpoint_epochX_iterY.pth
            save_path = os.path.join(ckpt_path, '{}_checkpoint_epoch{}_iter{}.pth'.format(model_name, epoch, iters))
            torch.save(netG.state_dict(), save_path)
    
    def save_final_model(self, netG, best_metric_name):
        """Save the final best model as MODEL_final.pth"""
        ckpt_path = self.config.TRAIN.ckpt_dir
        model_name = getattr(self, '_comparison_name', 'PEAN').upper()
        final_path = os.path.join(ckpt_path, '{}_final.pth'.format(model_name))
        torch.save(netG.state_dict(), final_path)
        print(f'Final best model saved: {final_path} (best on {best_metric_name})')
