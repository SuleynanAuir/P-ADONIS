...
def plot_metrics_scatter(metrics_history, save_path='./ckpt/metrics_scatter.png'):
    """
    ...
    if not metrics_history or 'iters' not in metrics_history:
    ...
    # Extract data
    iters = metrics_history.get('iters', [])
    if len(iters) == 0:
        print("No iterations to plot")
        return
    
    psnr_easy = metrics_history.get('psnr_avg_easy', [])
    psnr_medium = metrics_history.get('psnr_avg_medium', [])
    psnr_hard = metrics_history.get('psnr_avg_hard', [])
    acc_easy = metrics_history.get('easy_aster', [])
    acc_medium = metrics_history.get('medium_aster', [])
    acc_hard = metrics_history.get('hard_aster', [])
    
    # Plot 1: PSNR vs Accuracy (Easy)
    if psnr_easy and acc_easy:
        axes[0, 0].scatter(psnr_easy, acc_easy, alpha=0.6, c='green', s=50)
        axes[0, 0].set_xlabel('PSNR (Easy)')
        axes[0, 0].set_ylabel('Accuracy (Easy)')
        axes[0, 0].set_title('PSNR vs Accuracy - Easy')
        axes[0, 0].grid(True, alpha=0.3)
    
    # Plot 2: PSNR vs Accuracy (Medium)
    if psnr_medium and acc_medium:
        axes[0, 1].scatter(psnr_medium, acc_medium, alpha=0.6, c='orange', s=50)
        axes[0, 1].set_xlabel('PSNR (Medium)')
        axes[0, 1].set_ylabel('Accuracy (Medium)')
        axes[0, 1].set_title('PSNR vs Accuracy - Medium')
        axes[0, 1].grid(True, alpha=0.3)
    
    # Plot 3: PSNR vs Accuracy (Hard)
    if psnr_hard and acc_hard:
        axes[1, 0].scatter(psnr_hard, acc_hard, alpha=0.6, c='red', s=50)
        axes[1, 0].set_xlabel('PSNR (Hard)')
        axes[1, 0].set_ylabel('Accuracy (Hard)')
        axes[1, 0].set_title('PSNR vs Accuracy - Hard')
        axes[1, 0].grid(True, alpha=0.3)
    
    # Plot 4: Accuracy comparison across difficulties
    if acc_easy and acc_medium and acc_hard:
        x_pos = np.arange(len(iters))
        axes[1, 1].plot(x_pos, acc_easy, 'o-', label='Easy', color='green', alpha=0.7)
        axes[1, 1].plot(x_pos, acc_medium, 's-', label='Medium', color='orange', alpha=0.7)
        axes[1, 1].plot(x_pos, acc_hard, '^-', label='Hard', color='red', alpha=0.7)
        axes[1, 1].set_xlabel('Validation Step')
        axes[1, 1].set_ylabel('Accuracy (%)')
        axes[1, 1].set_title('Accuracy Across Difficulties')
        axes[1, 1].legend()
    ...
    plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f'Metrics scatter plot saved to: {save_path}')
