...
def main(config, args):
    ...
    mission = SwinIRTextSRInterface(config, args)
    if args.test:
        mission.test()
    else:
        # Ensure SwinIR checkpoint directory exists
        os.makedirs(config.TRAIN.ckpt_dir, exist_ok=True)
        log_path = os.path.join(config.TRAIN.ckpt_dir, "log.csv")  # Use standard log.csv name
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SwinIR TextSR training entry point")
